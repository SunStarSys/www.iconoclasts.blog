...
<li class="nav-item{% if path|starts_with:"/powered-by.md" %}
        ...
        <form id="search" action="/dynamic/search{% ifequal path|dirname "/" %}{% else %}{{ path|dirname
            }}{% endifequal %} clase="d-flex form-inline derecha" método="Obtener"
          <input type="hidden" name="lang" value="{{ lang }}" >
          <input type="hidden" name="markdown_search" value="1" >
          <input class="form-control me-2" type="text" name="regex"
               marcador de posición="Búsqueda recursiva PCRE" valor="{{ regex }} />&nbsp;<button type="submit" name="submit" value="1" class="btn btn-outline-danger"><i class="fa fa-search"></i></button>
	    </form>
  </div>
</header>
