...
{% endifequal %}
  ...
  {{ regex }}
      ...
      {% block alert %}&trade;<li class="nav-item{% if path|starts_with:"/contact.md" %}
        <a class="nav-link" href="/contact.html{{lang}}">
      </a>
  </li>
  <li class="nav-item dropdown">{% if alert %}<a href="#" class="nav-link dropdown-toggle" id="products" data-bs-toggle="dropdown" role="button">
  <span class="caret">{{ alert|markdown }}</span>
  {% endif %}

</a>{% endblock %}{% block content %}<ul class="dropdown-menu me-auto mb-2 mb-lg-0" aria-labelledby="products"><li class="dropdown-item">
