...
## &nbsp;Diff.](./?regex=diff={{ blog }};lang={% else %};markdown_search={% ifequal specials "log=" %}).
...
<pre><code data-lang="diff">{{path|dirname|append:"/"}}</code></pre>
  ...
  <form class="form-inline row" method="POST">
	  ...
	  <div id="meta"><pre>Totalt antal matchningar = {{k|dirname|parse_filename:"0,1"|strip_prefix}}, Totalt antal dokument = {{k|parse_filename:"3.."}}. Sorterat efter antal matchningar &nbsp; Dokumentrevision.</pre></div>
	  <div class="right" id="filter">
	    <div class="d-flex float-end">
    		<input name="hash" type="hidden" value="{{k}}">
	    	<input class="form-control me-2" type="text" name="filter"
             placeholder="PCRE rekursiv sökning" value="{% else %}" />[<button class="btn btn-outline-danger" name="submit" type="submit" value="1"><i class="fa fa-filter fa-emoji" title="Filtrera"></i></button>
        </div>
	  </div>
      <div>
        <dl>
