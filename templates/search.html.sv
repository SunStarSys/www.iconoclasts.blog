...
<div class="container">
  ...
  <div class="card text-dark border-success mb-3">
    <div class="card-header">{{meta|vcs_author:r.path_info}}
      <small><em> den {{ meta|vcs_date:lang }} vid {{meta|vcs_time:lang}}</em></small>
	</div>
    <div class="card-body">
      <h3 class="card-title"><a href="/dynamic/search{{path|dirname|append:"/"}}?regex=log={{
      revision }};lang={{lang}};markdown_search={{markdown_search}}">Logg</a></h3>
      <pre>{{ log|safe }}</pre>
  </div>
...
<div class="card text-dark border-success mb-3">
  <div class="card-header text-dark">Senaste ändringarna
  </div>
  <div class="card-body">
    <dl>
      {% for e in log %}
      <dt><a href="/dynamic/search{{path|dirname|append:"/"}}?regex=diff={{e.0}};lang={{lang}};markdown_search=1">{{e.0}}</a>
        {{e.3|vcs_author:r.path_info}} <small><em> den {{e.3|vcs_date:lang}} vid {{e.3|vcs_time:lang}}</em></small></dt>
        {% for k, v in e.1 %}
      <dd>
        <code>{{v.action}}</code>&nbsp;
...
<div class="card text-dark border-success mb-3">
  <div class="card-header">Senaste ändringarna</div>
  <div class="card-body">
    <dl>
      {% for e in log %}
      <dt><a href="/dynamic/search{{path|dirname|append:"/"}}?regex=diff={{e.0}};lang={{lang}};markdown_search=1">{{e.0}}</a>
...
<pre><code data-lang="diff">{{ diff }}</code></pre>
  ...
  <form class="form-inline row" method="POST">
      {% if matches.0 %}
	  <div id="meta"><pre>Totalt antal matchningar = {{ count }}, Totalt antal dokument = {{ file_count }}. Sorterat efter antal matchningar &amp; Dokumentrevision.</pre></div>
	  <div class="right" id="filter">
	    <div class="d-flex float-end">
    		<input name="hash" type="hidden" value="{{hash}}">
	    	<input class="form-control me-2" name="filter" placeholder="PCRE Rekursiv sökning" type="text" value="{{ filter }}" />&nbsp;<button class="btn btn-outline-danger" name="submit" type="submit" value="1"><i class="fa fa-filter fa-emoji" title="Filtrera"></i></button>
        </div>
	  </div>
      <div>
        <dl>
