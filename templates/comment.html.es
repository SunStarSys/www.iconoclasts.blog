      <article id="article-{{key}}" {% if muted %}class="text-muted"{% endif %}>
        <header>
          <h6 class="card-title" id="{{key}}">
            ...
            por {{content|vcs_author:lang|removetags:"script style"|removeattrs:"on\w+ style class"}}
            activado <em><time>{{content|vcs_date:lang|removetags:"script style"|removeattrs:"on\w+ style class"}}</time></em>
            {% if muted %}
            {% else %}
            {% if important %}
            &nbsp;
