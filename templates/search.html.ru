...
## &nbsp;Разница](./?regex=diff={{ blog }};язык={% else %};markdown_search={% ifequal specials "log=" %}).
...
<pre><code data-lang="diff">{{path|dirname|append:"/"}}</code></pre>
  ...
  <form class="form-inline row" method="POST">
	  ...
	  <div id="meta"><pre>Всего соответствий = {{k|dirname|parse_filename:"0,1"|strip_prefix}}, Всего документов = {{k|parse_filename:"3.."}}. Заказано по числу совпадений &nbsp; Версия документа.</pre></div>
	  <div class="right" id="filter">
	    <div class="d-flex float-end">
    		<input name="hash" type="hidden" value="{{k}}">
	    	<input class=form-control me-2 type=text name=filter
             placeholder=«Рекурсивный поиск PCRE» value=»{% else %}" />[<button class="btn btn-outline-danger" name="submit" type="submit" value="1"><i class="fa fa-filter fa-emoji" title="Фильтр"></i></button>
        </div>
	  </div>
      <div>
        <dl>
