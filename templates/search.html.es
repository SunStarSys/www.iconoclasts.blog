...
{% block title %}{{title|safe}} /{% if filter %}{{filter}}{% else %}{{regex}}{% endif %}/gi{% endblock %}
{% block headers %}
...
<div class="container">
  ...
  {% endfor %}
  {% for k in keywords %}
  <a href="./?regex=%23{{k|urlencode}};lang={{lang}};markdown_search={{markdown_search}}"><span class="badge bg-danger text-white">#{{k}}</span></a>
  &nbsp;
  {% endfor %}
...
{{dep_dot}}
  ...
  <div class="card text-dark border-success mb-3">
    <div class="card-header">{{meta|vcs_author:r.path_info}}
      <small><em> activado {{ meta|vcs_date:lang }} a las {{meta|vcs_time:lang}}</em></small>
	</div>
    <div class="card-body">
      <h3 class="card-title"><a href="/dynamic/search{{path|dirname|append:"/"}}?regex=log={{ revision }};lang={{lang}};markdown_search={{markdown_search}}">Registro</a></h3>
      <pre>{{ log|safe }}</pre>
  </div>
  ...
  <div class="card-body">
    <dl>
      ...
      <dt><a href="/dynamic/search{{path|dirname|append:"/"}}?regex=diff={{e.0}};lang={{lang}};markdown_search=1">{{e.0}}</a>
        {{e.3|vcs_author:r.path_info}} <small><em> activado {{e.3|vcs_date:lang}} a las {{e.3|vcs_time:lang}}</em></small></dt>
        {% for k, v in e.1 %}
      <dd>
        <code>{{v.action}}</code>&nbsp;
    ...
    <div class="right" id="filter">
      <div class="d-flex float-end">
        ...
        <input class="form-control me-2" type="text" name="filter"
             placeholder="PCRE Recursive Search" value="{{ filter }}" />&nbsp;<button type="submit" name="submit" value="1" class="btn btn-outline-danger"><i class="fa fa-filter fa-emoji" title="Filter"></i></button>
      </div>
    </div>
{% for w in weblog %}
      ...
      ctx,
      {
          ...
          options: {
              ...
              plugins: {
                  title: {
                      display: true,
                      text: (total / 60).toFixed(0) + " build minutes this month / " + count + " total builds (average build duration is " + (total /count).toFixed(0) + " s)",
                  }
              },
