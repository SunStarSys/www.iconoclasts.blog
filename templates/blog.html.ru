   ...
   <div class="breadcrumbs">{{ breadcrumbs|safe }}&nbsp;&nbsp;<a href="javascript:void(location.href='https://cms.sunstarsys.com/redirect?uri='+escape(location.href)+';action=edit')"><img src="/images/edit.png" /></a></div>
     ...
     <div class="essay col-lg-12 container">
       <div style="color:#aaa">
        ...
        <em>
			<a href="/dynamic/search{{path}}?regex=diff=r{{content|vcs_revision}};lang={{lang}};markdown_search=1">Последнее обновление</a> от <b>{{ content|ssi|vcs_author:lang }}</b> на <b>{{ content|ssi|vcs_date:lang }}</b>
{% ifequal repos "public" %}
			   &nbsp;
			   <a href="https://github.com/SunStarSys/{{website}}/blob/trunk/content{{path}}"><i class="fa-brands fa-sm fa-github"></i>&nbsp;источник</a>
{% endifequal %}
			 </em>
         </small>
         <div class="right">
           <a href="https://cms.sunstarsys.com/redirect?uri=https://{{website}}{{path}};action=watch" id="watch">
             <i class="fa fa-eye fa-emoji" title="наручные часы"></i>
           </a>
...
<script async="" type="module">
  if (document.cookie.indexOf(can_search) >= 0) {
    const response = ожидание fetch("/dynamic/search){{path|dirname}}/?regex=watch=;lang={{lang}};markdown_search=1;as_json=1",
