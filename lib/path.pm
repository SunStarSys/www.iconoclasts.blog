...
walk_content_tree {
  ...
  for my $lang (qw/en es de fr ru sv he zh-TW ar ko ja pt-BR/) {
    ...
    if (s!/index\.html\.$lang$!!) {
      $dependencies{"$_/index.html.$lang"} = [
        grep s/^content// && !archived && utf8::encode($_),
  ...
  and do {
    while  (my ($k, $v) = each %{$facts->{dependencies}}) {
      push @{$dependencies{$k}}, grep $k ne $_, grep s/^content// && !archived && utf8::encode($_), map glob("'content'$_"), ref $v ? @$v : split /[;,]?\s+/, $v;
    }

    open my $fh, "<:raw", "lib/acl.yml" or die "Can't open acl.yml: $!";
    push @acl, @{Load join "", <$fh>};
    my %cache;
    for (grep utf8::encode($_), glob("content/*/index.md.*")) {
