...
Det är nu en tvetydig katastrof av tekniska regex-defekter varje gång en författare / implementerare måste gissa vad parsern är tänkt att göra med sekvenser av isolerade dollartecken som förekommer inom ett enda stycke av nedsättningsprosa, som när franska kanadensiska finansiella matematiker diskuterar två priser i två olika valutor, säger `$`1,50 och 2 00`$` CA, den senare är den franska kanadensiska dollarn (visst framtagen, men det illustrerar komplexiteten). Lycka till med det (är `$` år $$\LaTeX$$ token, eller är det något helt annat) om du gör en mass-GFM-konvertering av äldre dokumentformat till en icke-GitHub plattform. Det kommer bara "studera" när det renderas i Visual Studio, om det ens fungerar alls.
