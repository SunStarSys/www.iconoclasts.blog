...
Min erfarenhet av e-postlista programvara kretsar kring [qmail](http://cr.yp.to/qmail.html) och [ezmlm-idx](http://untroubled.org/ezmlm/). Med några få små skript kunde jag stödja en mängd nya användningsfall som inte stöds av `ezmlm-idx` själv.  Funktioner som är allmänt användbara utanför [Apache Software Foundation](http://www.apache.org) anges nedan.  Så här använder du dessa filer [layout]

### BATV och SRS

[lagerplats/avsändare-demonger](ezmlm/bin/sender-demunger.pl) är ett litet wrapper-skript som aktiverar `BATV` och `SBS` `AVSÄNDARE` avsugning för `ezmlm-idx`. Om du vill använda den lägger du bara till den som ett prefix på alla rader i `</editor />` och `</manager />` block inom `.ezmlmrc` och springa `ezmlm-tillverkning -+` på dina listor, eller i en nypa förutsatt att du inte kommer att köra `ezmlm-make` igen i dina listor, redigera `redaktör` och `bas`

[BATV](http://en.wikipedia.org/wiki/Bounce_Address_Tag_Validation) och [SBS](http://en.wikipedia.org/wiki/Sender_Rewriting_Scheme) utgör unika problem för `ezmlm-idx` eftersom det, till skillnad från andra e-postprogram, fungerar på `E-POST FRÅN` del av `SMTP`
...
<div class="alert alert-info">
   <div class="alert-header">Obs!</div>
