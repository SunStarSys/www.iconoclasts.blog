...
Det senare konceptet är vad vi kommer att använda när vi diskuterar *beroendegrafens *topologi `$$\tau$$` associerad med arbetsytan `$$X$$` av källfiler under webbplatsens `innehåll/` underkatalog (här) `$$(Y,\sigma)$$` är `$$\mathbb{R}^n$$` med sin metriska topologi för `$$n \in \{2,3\}$$`och kanterna på `$$X$$` är icke-korsande, riktade jordanska kurvor som ansluter en fil `$$x \in X$$` till sin uppsättning filer som `$$x$$` beror på: `$$\set{x^\prime \in X | x \rightarrow x^\prime}$$`).

Att {# lede #}en tydlig förståelse för din webbplats *beroende diagram *kommer att se till att du kan maximera prestandan hos vår byggteknik i stor skala{# lede #}. Vi tar den information du lämnar till `%path::beroenden` under byggets belastning av din webbplats `lib/path.pm` fil, konstruera en omvänd karta över beroende filer och använd *den omvända kartan* för att fastställa den fullständiga korpus av filer som ska byggas för en given `sv-bekräftelse` Du gör till vårt system.
...
Vår webbplats består för närvarande av `240 källfiler` om `innehåll/`. Här är en `85 hörn x 465 kanter`, rullningsbar, tvådimensionell dirigerad diagramrepresentation av en aktuell ögonblicksbild av sidberoenden på engelska på **vår webbplats** ([använda GraphViz `prick`](https://github.com/SunStarSys/orion/blob/master/deps2dotcfg.pl)):
...
Här är en `240+ hörn x 3859 kanter`, aktuellt diagram med fågelperspektiv i det engelska *link topology*-diagrammet för **vår plats** ([använda GraphViz `tvåhörning`](https://github.com/SunStarSys/orion/blob/master/links2dotcfg.pl)):
...
- användbart för att undvika att använda ett stort värde (3+) av `quick_deps` om `@path::mönster` postens argument hashref, vilket kan påverka prestanda
