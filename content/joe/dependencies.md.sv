...
dependencies: '*.md.sv'
keywords: arkitektur,inkrementell,webbplats,generering,byggen,beroenden,topologi,wiki,graph,smart
published: ~
status: verifierad=22964
...
* Hantering av beroenden för smart innehåll&trade;* handlar om cirkeln av idéer relaterade till att ge stöd och underlättande för *inkrementella byggen*, samtidigt som man håller sig till **Innehållsnormaliseringsprincipen** &mdash; det där [*permalänkar*](https://en.wikipedia.org/wiki/Permalink) bör vara den enda källan till sanning, oavsett hur deras innehåll kurateras genom källträdet och resulterande byggartefakter.
...
Webbplatsen Apache <https://www.OpenOffice.Org> kunde till exempel bygga sina 40K+-filer med den ursprungliga Apache-versionen av det här byggsystemet, med fullt integrerat stöd för inkrementella byggen. &mdash; utan konfigurerade beroenden &mdash; genom att göra smart användning av traditionell SSI-teknik ensam.

Som standard bygger vårt byggsystem endast de filer som du har ändrat, utan problem för beroenden inom filen (om du inte anger dem i `%path::beroenden` &mdash; mer om detta nedan). Om filen du ändrade finns i `mallar/` eller `lib/` katalog, en fullständig webbplatsbygge utlöses istället.

## Weaving Your Website's *Dependency Graph * Tillsammans

Matematiskt en *Topologi* `$$\tau$$` är en fullständig specifikation av de *öppna* delmängderna i ett utrymme `$$X$$`, vars syfte är att ange närhetsrelationerna mellan *punkter* `$$x$$` från rymden `$$X$$`. När `$$X$$` är ett diagram, en *topologi* `$$\tau$$` för `$$X$$` uppgår till att ange kanterna som förbinder diagrammets brytpunkter tillsammans (här ses brytpunkter som *punkter* för `$$X$$`, och de anslutande kanterna bestämmer områdena för dessa punkter som *grundläggande öppna uppsättningar* för topologin).  En *riktad graftopologi* är i huvudsak samma sak, men innehåller en hänvisning till en topologisk inbäddning av `$$(X,\tau)$$` till ett större topologiskt rum `$$(Y,\sigma)$$` , där inbäddningens kantanslutningar representeras av riktningskurvor som inte korsar varandra (Jordanien).
...
#### Traditionell [Serversidan innehåller](https://httpd.apache.org/docs/2.4/howto/ssi.html) (SSI).
...
- användbar för att undvika att använda ett stort värde (3+) av `quick_deps` i en `@path::mönster` inmatningens argument hashref, vilket kan påverka prestanda
...
- vår [Wiki för Orion Enterprise](/orion/) Systemet stöder inte dem
...
1. [Identifiering av kluster i webbdiagrammet baserat på länktopologi](https://ieeexplore.ieee.org/abstract/document/1214919) Sjunde International Database Engineering and Applications Symposium, 2003. förfaranden.

2. [Hämta webbforum från länktopologi](https://dl.acm.org/doi/pdf/10.1145/276627.276652) Den nionde ACM-konferensen om hypertext och hypermedia: länkar, objekt, tid och rum &mdash; struktur i hypermedia system: länkar, objekt, tid och rum &mdash; struktur i hypermediasystem. 1998.

3. [Om fördelarna och gränserna för inkrementella konfigurationer av programvara: en undersökande studie](https://dl.acm.org/doi/abs/10.1145/3510003.3510190) ICSE '22: Förfaranden vid den 44:e internationella konferensen om programvaruteknik, maj 2022

4. [Mot inkrementell utveckling av programvarukonfigurationer](https://dl.acm.org/doi/10.1145/3510455.3512792) ICSE-NIER '22: Proceedings of the ACM/IEEE 44th International Conference on Software Engineering: New Ideas and Emerging Results, maj 2022

5. [Ett ljud och optimalt inkrementellt byggsystem med dynamiska beroenden](https://dl.acm.org/doi/10.1145/2814270.2814316) OOPSLA 2015: Proceedings of the 2015 ACM SIGPLAN International Conference on Object-Oriented Programming, Systems, Languages, and Applications oktober 2015
