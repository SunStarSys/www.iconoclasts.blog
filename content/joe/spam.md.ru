...
Как я писал о [в другом месте](https://sunstarsys.com/clients/apache)один из лучших инструментов с открытым исходным кодом, доступных для арсенала средств борьбы со спамом администратора почты, является [qpsmtpd](http://en.wikipedia.org/wiki/Qpsmtpd), который является просто фронтенд-сервером входящей почты, который нуждается в полнофункциональном почтовом сервере, таком как `qmail`, `Отправить сообщение`, `экзотический`или `постфикс` для исходящей доставки.  Что `qpsmtpd` Это очень похоже на то, что `httpd` для веб-серверов: он вводит настраиваемые крючки в каждую фазу `SMTP`
