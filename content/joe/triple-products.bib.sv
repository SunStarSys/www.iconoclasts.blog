@article{MS67,
  title={Kurvatur och Lapplands egenvärden},
  journal={Tidskrift för differentialgeometri},
  author={McKean, Henry P. och Singer, Isadore M.},
  volume={1},
  number={1-2},
  pages={43-69},
  year={1967},
  publisher={Lehigh University}
}
}
@book{JC19,
  author={Conway, John B.},
  title={En kurs i funktionell analys},
  volume={96},
  year={2019},
  publisher={Springer}
}
}
@article{CS92,
 author={Conway, John H och Sloane, N. J. A.},
 title={Fyrdimensionella galler med samma teta-serie},
 journal={Internat Matematik. Res. Meddelanden},
 volume={4},
 year={1992},
 pages={93-96}
}
}
@article{DH11,
  author={Datchev, Kiril och Hezari, Hamid},
  title={Omvända problem i spektralgeometri},
  journal={Inversa problem och tillämpningar: Inside Out II},
  volume={60},
  year={2011},
  pages={455-486}
}
}
@book{FBZ04,
 author={Frenkel, Edward och Ben-Zvi, David},
 title={Vertex algebror och algebraiska kurvor},
 volume={88},
 publisher={Amerikansk matematisk soc.},
 year={2004}
}
}
@online{TF17,
  author={Franke, Tobias},
  title={Heltal för trippelprod\ukt},
  date={2023-09-01},
  url={https://www.tobias-franke.eu/log/2017/04/19/triple-products.html}
}
}
@article{CG93,
  author={Gordon, Carolyn},
  title={Isospektral stängda Riemanniska grenrör som inte är lokalt isometriska},
  journal={Tidskrift för differentialgeometri},
  volume={37},
  number={3},
  year={1993},
  pages={639-649}
}
}
@article{GK80,
  author={Guillemin, Victor och Kazhdan, David},
  title={Vissa inversa spektrala resultat för negativt böjda n-mångfalder},
  journal={Symposier i ren matematik},
  volume={36},
  pages={301-312},
  year={1980}
}
}
@article{LH68,
  author={Hörmander, Lars},
  title={Den spektrala funktionen hos en elliptisk operatör},
  journal={Matematik Tidigare och nuvarande Fourier Integral Operatorer},
  publisher={Springer Berlin Heidelberg},
  year={1968},
  pages={217-242}
}
}
@book{AK01,
  author={Knapp, Anthony W},
  title={Representationsteori för semisimplegrupper: en översikt baserad på exempel},
  publisher={Hotell nära Princeton University Press},
  year={2001}
}
}
@online{LS18,
  author={Lu, Jianfeng och Steinerberger, Stefan},
  title={På punktvisa produkter av elliptiska egenfunktioner},
  archivePrefix={arXiv},
  eprint={1810.01024},
  year={2018}
}
}
@proceedings{JM64,
  author={Milnor, John},
  title={Egenvärden för Laplace-operatören på vissa grenrör},
  journal={Förfaranden vid Vetenskapsakademien},
  volume={51},
  number={4},
  year={1964},
  pages={542-542}
}
}
@article{NRR22,
  author={Nilsson, Erik och Rowlett, Julie och Rydell, Felix},
  title={Isospektralt problem för platt tori ur tre perspektiv},
  journal={Kommuniké från American Mathematical Society},
  volume={60},
  number={1},
  year={2023},
  pages={39-83}
}
}
@book{RS75,
  author={Reed, Michael och Simon, Barry},
  title={Metoder för modern matematisk fysik II: Fourieranalys, självförverkligande},
  volume={2},
  publisher={Elsevier},
  year={1975}
}
}
@article{PS94,
  author={Sarnak},
  title={Integraler av produkter av egenfunktioner},
  journal={IMRN},
  volume={6},
  year={1994},
  pages={251-260}
}
}
@phdthesis{AS94,
  author={Schiemann},
  title={Ternäre positiv definitiv quad\-ratische Formen mit gleichen Darstellungs\-zahlen},
  publisher={Bonner Mathe\matische Schriften},
  school={Universität Bonn},
  volume={268},
  year={1993}
}
}
@article{TS85,
  author={Sunada (ort)},
  title={Riemannbeläggningar och isospektrala grenrör},
  journal={Matematikens årtal},
  volume={121},
  number={1},
  year={1985},
  pages={169-186}
}
}
@article{ST80,
  author={Tanno, Shukichi},
  title={En karakterisering av de kanoniska sfärerna av spektrumet},
  journal={Matematik. Ö.},
  volume={175},
  number={3},
  year={1980},
  pages={267-274}
}
}
@book{MT13,
  author={Taylor, Michael},
  title={Partiella differentialekvationer II: kvalitativa studier av linjära ekvationer},
  volume={116},
  publisher={Springer Science \&amp; Affärsmedia},
  year={2013}
}
}
@article{HW11,
  author={Weyl, Hermann},
  title={Über die asymp\-totische Ver\-teilung der Eigen\-werte},
  journal={Nach\-richten der König\-lichen Gesell\-schaft der Wissen\-schaften zu Göttingen},
  year={1911},
  pages={110-117}
}
}
@article{EW22,
  author={Wyman, Emmett L},
  title={Trianglar och trippelprodukter av Laplace egenfunktioner},
  journal={Verifikation för funktionell analys},
  volum={282},
  number={8},
  year={2022},
  doi = {https://doi.org/10.1016/j.jfa.2022.109404}
}
}
@article{GZ97,
  author={Zhou, Gengqiang},
  title={Kompakthet hos isospektrala kompakta grenrör med avgränsade krökar},
  journal={Stilla havet J. Matematik},
  volume={181},
  number={1},
  year={1997},
  pages={187-200}
}
