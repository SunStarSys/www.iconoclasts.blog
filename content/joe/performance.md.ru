...
Даже беззависимый сценарий bash – это работоспособное решение для многих основных задач. Вот тот, который я написал для фирмы дополненной реальности [Волшебный скачок](https://magicleap.com) много лет назад, чтобы заменить неуклюжий [OpenGrok](https://oracle.github.io/opengrok/) сервис с чем-то, что использует многопроцессорную параллелизацию с `xargs -P`, и поддерживает [Pусский](https://www.pcre.org/) поиск по простому `Эмак`/`Вим`
