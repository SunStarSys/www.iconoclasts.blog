...
dependencies: '*.md.es'
keywords: arquitectura,incremental,sitio web,generación,construcciones,dependencias,topología,wiki,gráfico,smart
published: ~
...
-----
## Tejiendo su sitio web's *Gráfico de dependencia* juntos

Matemáticamente, una *Topología* `$$\tau$$` es una especificación completa de los subconjuntos *abiertos* de un espacio `$$X$$`, cuyo objetivo es indicar las relaciones de proximidad entre *puntos* `$$x$$` del espacio `$$X$$`. Cuándo `$$X$$` es un gráfico, una *topología* `$$\tau$$` para `$$X$$` equivale a especificar los bordes que conectan los vértices del gráfico juntos (aquí los vértices se ven como los *puntos* de `$$X$$`, y los bordes de conexión determinan los barrios de esos puntos como *conjuntos abiertos básicos* para la topología).  Una topología de gráficos dirigida es esencialmente lo mismo, pero incorpora una referencia a una incrustación topológica de `$$(X,\tau)$$` en un espacio topológico más amplio `$$(Y,\sigma)$$` , donde la incrustación'Las conexiones de borde están representadas por curvas direccionales, no intersectivas (Jordania).

Este último concepto es lo que utilizaremos al discutir el gráfico de dependencia'topología s* `$$\tau$$` asociado al espacio `$$X$$` de archivos fuente debajo de su sitio's `contenido/` subdirectorio (aquí `$$(Y,\sigma)$$` es `$$\mathbb{R}^n$$` con su topología métrica para `$$n \in \{2,3\}$$`y los bordes de `$$X$$` No son intersecciones, curvas dirigidas Jordan que conectan un archivo `$$x \in X$$` a su conjunto de ficheros sobre los que `$$x$$` depende de: `$$\set{x^\prime \in X | x \rightarrow x^\prime}$$`).

Que tiene {# lede #}una comprensión clara de su sitio web's *gráfico de dependencia* se asegurará de que pueda maximizar el rendimiento de nuestra tecnología de compilación a escala{# lede #}. Tomamos la información que usted proporciona a `%path::dependencias` durante la construcción'carga de su sitio web's `lib/path.pm` archivo, construir un mapa inverso de archivos dependientes y utilizar *ese mapa inverso* para determinar el corpus completo de archivos que se van a crear para cualquier `confirmación svn` a nuestro sistema.

Es'Es importante tener en cuenta que las relaciones de dependencia entre los archivos de origen pueden y deben ser capturadas por completo por `%path::dependencias` hash durante el sistema de creación's carga de inicio de `lib/path.pm` de su árbol de origen, que es cómo las vistas incorporadas contenidas en nuestro `SunStarSys::Ver` El paquete Perl está destinado a funcionar. El `walk_content_tree`, `archivado`y `seed_file_deps` funciones de utilidad importables de `SunStarSys::Util` son útiles para construir el `%path::dependencias` hash, con soporte integrado para gestionar una caché de dependencias para acelerar las compilaciones incrementales a escala.

Aquí'esa parte de nuestra vida [`lib/path.pm`]({{snippetA.pretty_uri}}):

[snippet:lang=perl:repo=SunStarSys/www:branch=trunk:path=lib/path.pm:token=#snippet]

Por favor, revisa ese código para obtener ideas sobre cómo quieres que funcione tu sitio web. Sí, hay cierta complejidad razonable (que involucra tanto a Perl'S expresiones regulares y Perl'cáscara UNIX s `glob` interfaces, de forma muy precisa) sobre cómo `%path::dependencias` se construye en ese archivo, pero en lugar de simplemente ver esto como un trabajo de optimización, en su lugar, mirarlo como proporcionar los ingredientes básicos necesarios para construir los principales aspectos de la topología *link *de una manera automatizada y generada dinámicamente.

Dónde están las entradas en `%path::dependencias` ¿Origen? Si no nace de una invocación de `walk_content_tree { seed_file_deps ... }`, (que básicamente se sumerge en sus archivos de origen Markdown' encabezados y contenido), entonces solo están codificados en `lib/path.pm` en tiempo de carga.

### Los gráficos de dependencias cíclicas son la norma

Nuestro sitio actualmente consiste en `240 archivos de origen` en `contenido/`. Aquí'es `85 vértices x 465 bordes`, representación de gráfico dirigida bidimensional desplazable de una instantánea reciente de las dependencias de la página en inglés en **nuestro sitio** ([uso de GraphViz's `punto`](https://github.com/SunStarSys/orion/blob/master/deps2dotcfg.pl)):
...
Muy complejo, incluso para un sitio web pequeño como este! Muchas intersecciones de borde al tomar `$$n=2$$` (evitable en dimensión `$$n=3$$`). De particular importancia es el conjunto principal de dependencias cíclicas densas en los archivos no archivados en nuestro sitio's `/ensayos/` directorio, hacia la parte inferior-centro-derecha del gráfico, que es lo que un buen sitio de blogs'El gráfico de dependencias debe ser similar. Estas dependencias se trazan en `curvas rojas` en la imagen.
...
**¡No!** De hecho, la topología *link* de su sitio web es un asunto completamente separado del árbol de origen's *gráfico de dependencia*.  Un motor de búsqueda buscará naturalmente la topología *link*, pero no tiene información sobre el gráfico *dependent*.

Aquí's a `Más de 240 vértices x 3859 bordes`, gráfico actual de ojos de pájaro del gráfico en inglés *link topology* para **nuestro sitio** ([uso de GraphViz's `twopi`](https://github.com/SunStarSys/orion/blob/master/links2dotcfg.pl)):
...
- **gran** para podar su sitio web's *gráfico de dependencia* hasta un tamaño manejable sin sacrificar la latencia de entrega de páginas
...
- **lusura** para volver a textualizar páginas web completas en una ubicación diferente en la raíz de documentos's jerarquía
...
- reescribe las URL relativas a las URL absolutas en la ruta de destino's contenido incluido
...
- útil para evitar el uso de un gran valor (3+) de `quick_deps` en un `@path::patrones` entrada'hashref de argumento s, que puede afectar al rendimiento
...
- Las categorías son estrictamente aditivas (es decir, eliminar una categoría de una página de origen)'los encabezados no lo eliminarán de esa categoría en el sitio activo),
- generado bajo demanda
- eliminar todas las categorías en una sola confirmación es una excelente manera de sincronizarlas con las especificaciones exactas en todas las páginas de origen' encabezados, sin destruir el contenido de categoría conservado en el sitio activo
...
Existen estructuras de datos y relaciones interesantes que aún no se han descubierto al tratar con un sitio web.'s *gráfico de dependencia* desde una perspectiva de rendimiento de construcción, que es un área de interés mucho más nueva que la literatura de investigación que profundiza en las estructuras de datos y las emisiones asociadas que rodean *topología de enlace *<sup>1,2</sup>.
