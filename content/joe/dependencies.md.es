...
*Gestión de dependencias de contenido inteligente&trade;* trata sobre el círculo de ideas relacionadas con el apoyo y la facilitación de *construcciones incrementales*, al tiempo que se mantiene fiel al **Principio de Normalización del Contenido** &mdash; que [*permalinks*](https://en.wikipedia.org/wiki/Permalink).
...
Nuestro sitio consiste actualmente en `240 archivos de origen` en `contenido/`. Aquí hay un `85 vértices x 465 bordes`, representación de gráfico dirigida bidimensional desplazable de una instantánea reciente de las dependencias de página en inglés en **nuestro sitio** ([uso de GraphViz `punto`](https://github.com/SunStarSys/orion/blob/master/deps2dotcfg.pl).
...
Por supuesto, nuestro [Wiki de Orion Enterprise](/orion/).
...
Aquí hay un `Más de 240 vértices x 3859 bordes`, gráfico actual del gráfico *link topology* en inglés para **nuestro sitio** ([uso de GraphViz `twopi`](https://github.com/SunStarSys/orion/blob/master/links2dotcfg.pl).
...
#### Traditional [Incluye del lado del servidor](https://httpd.apache.org/docs/2.4/howto/ssi.html).
...
- nuestro [Wiki de Orion Enterprise](/orion/).
...
Las compilaciones incrementales convencionales para proyectos de desarrollo de software puro siguen siendo un tema candente. La investigación cubierta en <sup>3,4</sup> se publicó en octubre de 2022, aproximadamente un mes antes de que se esperara que este ensayo se completara. El sistema de compilación *pluto*<sup>5</sup> tiene características muy similares a las nuestras (la compilación en sí misma puede regenerar y reconstruir dependencias de forma dinámica).
