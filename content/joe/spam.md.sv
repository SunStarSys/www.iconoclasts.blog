...
Som jag skrev om [någon annanstans](https://sunstarsys.com/clients/apache), ett av de bästa verktygen för öppen källkod som finns tillgängliga för en e-postadministratörs arsenal av skräppostverktyg är [qpsmtpd](http://en.wikipedia.org/wiki/Qpsmtpd), som bara är en server för inkommande e-post på klientsidan som behöver en fullt fungerande e-postserver som `qmail`, `skicka e-post`, `brädd`, eller `postfix` för utgående leverans.  Vad `qpsmtpd` är väldigt likt det som `httpd` gjorde för webbservrar: det introducerar anpassningsbara krokar i varje fas av `SMTP`
