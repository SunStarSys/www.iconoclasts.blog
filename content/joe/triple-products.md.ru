...
С $$\set {\bar e^i}$$ Фурье также является основой для $$L^2(M,g)$$, ясно из уравнения $$(2)$$ этот $$\vec F(\bar f) = \bar{\vec F}(f)$$. Это означает, что на плотном наборе $$C(M)$$ (и) $$C(N)$$), мы установили $$\vec{F}$$ как изоморфизм Абеляна $$C^*$$ алгебры, и, таким образом, могут быть распространены на изоморфизм $$C(M)$$ и $$C(N)$$ в той же категории.
