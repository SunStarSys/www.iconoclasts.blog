...
Это означает, что на плотном наборе $$C(M)$$ (и) $$C(N)$$), мы установили $$\vec{F}$$ как изоморфизм Абеляна $$C^*$$ алгебры, и, таким образом, могут быть распространены на изоморфизм $$C(M)$$ и $$C(N)$$ в той же категории.
...
Тем не менее, чтобы применить наши [Теорема](#theorem)Очень важно, чтобы такая линейная карта $$B$$ быть $$B\in O(n,\Reals)$$ на решетке веса, потому что $$L^2$$ карта основы собственной функции
...
Это представление-теоретический счет [[AK01]](#AK01) Точно эквивалентно предыдущему развитию конгруэнтности *решетки* [[NRR22]](#NRR22) Традиционно используется для определения изометрических классов плоских тори. Фактически, матрица перекладывает такую линейную карту $$B\in O(n,\Reals)$$, как описано в предыдущем абзаце, **-это** противоположная риманновская изометрия между тори, как предусмотрено применением *теоремы представления Гельфанда-Наймарка* во время [Подтверждение](#proof-of-theorem) нашей [Теорема](#theorem).
