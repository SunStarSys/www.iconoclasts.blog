...
Denna funktion var helt avgörande för att stödja Apache massiva <https://OpenOffice.org> (OOo) webbplats.  RDBMS CMS Sun som ursprungligen tillhandahölls till OOo skulle falla över och dö även om du bara ville fixa ett stavfel. Däremot körde Apache CMS på en FreeBSD fängelse på baldr.apache.org: en måttligt tilldelad, [Dell 1950-låda som kördes på 8 CPU och 24 GB RAM med ett par speglade hårddiskar på 96 GB](https://web.archive.org/web/20120112034217/http://www.apache.org/dev/machines.html#:~:text=baldr) över flera fängelser, och flög genom arbetsflödet med relativ lätthet.
...
1. Gå fisk den sidan ut ur en github repo,
2. gaffel för repo,
3. redigera sidan,
4. Genomför förändringen,
5. Tryck på den,
6. Skapa ett PR
...
9. **vänta** för en beställare att hitta och granska det publicerade ändrade innehållet någonstans på [mellanlagringsplats](https://www.staging.openoffice.org/),
10. **vänta** så att beställaren flyttar upp den **hela platsen** till produktion
...
12. **vänta** så att gitpubsub pushar ut det nya innehållet till Apache:s edge-webbservrar.

Med Apache CMS (webgui), dela en committable "patch/diff." över e-post var en enkelklicksoperation för någon person på jorden, samt en enkelklicksoperation för att bekräfta + bygga + publicera för en beställare att tillämpa på projektet. Det hela kretsade kring delning av funktions-URL:er i kontexten av en live Markdown-redigerare med samtidiga HTML-förhandsgranskningar med dubbla rutor. De tillät en Apache committer på projektet att "klon" baldr-jail-hosted zfs-filsystemet för en bidragsgivares (icke-engagerade) utcheckning; och inspektera, ändra och bekräfta sedan den klonade utcheckningen av Apache-åtagaren själva **som uppdragsgivare och inte bidragsgivare**.  När den bekräftelsen inträffar byggde CMS inte bara den på *sekunder* (eftersom det bara bygger de ändrade filerna och deras handfull beroende filer), men det tillhandahöll också **länkar** till bygget och till den levande återgivningen av innehållet på mellanlagringsplatsen för granskning före befordran till produktion.
...
1. En bussfaktor på 1 (mig).

2. fasa ut FreeBSD ([OpenZFS](https://openzfs.org/wiki/Main_Page) körs på ubuntu),
...
Vad som hände sedan var verkligen anmärkningsvärt: Apache Infrastructure Team, omedelbart och ihärdigt under en period av veckor, satte Dave i den otacksamma positionen att förklara sina nu exklusiva lojaliteter, enligt dem: för mig och i förlängningen projektets **volontär gemenskap**, eller till **The ASF**.

Dave var den viktigaste innovatören och samarbetspartnern bakom skalbarhetsframgångarna för Apache CMS: s inkrementella byggteknik.  Han uppfann inte lösningarna, men han arbetade produktivt med mig om hur jag lade till skalbarhetsfunktionerna han behövde för att säkerställa högpresterande byggen från The Apache CMS som tillämpas på OpenOffice-webbplatsen - som vid den tiden levererade norr om 25M begär en dag! Tillsammans upptäckte vi en djupgående tillämpning av [SSI](https://en.wikipedia.org/wiki/Server_Side_Includes) i hans ansträngningar, som jag tror fortfarande genomförs i det JBake-templateringssystem som används i dag.

Tyvärr, om du tittar på den innehållsredigering som pågår med OpenOffice webbplats på GitHub nyligen, kan du tydligt se en massiv avstängning i aktivitet när Apache Infrastructure Team tvingade Dave att flytta dem från Apache CMS, och som ett resultat kanaliserade all bidragsaktivitet genom GitHub ensam.

Jag klandrar inte Dave för det fula val han var tvungen att göra, inte heller för det förutbestämda resultatet, men vi är uppenbarligen inte längre på talande villkor sedan den dagen.
...
Men inte besegrad. Jag hällde all den energin i byggnaden [Orion](https://sunstarsys.com/orion/features) till en oöverträffad, säkerhetsinriktad, världsledande konstruktion [KMS](wikis). Kolla in det om du tröttnar på att ha dina företagshemligheter stulna av script-kiddies; du kommer att älska det!
